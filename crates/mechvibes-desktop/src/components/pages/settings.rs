...
use crate::theme::{BuiltInTheme, Theme, use_theme};
...
use mechvibes_core::utils::auto_updater::{UpdateInfo, check_for_updates_simple};
use crate::utils::config::use_config;
use mechvibes_core::utils::constants::{APP_NAME, APP_NAME_DISPLAY};
use mechvibes_core::utils::time::format_relative_time;
use dioxus::prelude::*;
...
pub fn SettingsPage() -> Element {
    ...
    use_effect(move || {
        if let Some(saved_update) = mechvibes_core::utils::auto_updater::get_saved_update_info() {
            update_info.set(Some(saved_update));
        }
    });

    // Get current version for display
    let current_version = mechvibes_core::utils::constants::APP_VERSION;
                            ...
                            Toggler {
                                ...
                                on_change: {
                                    ...
                                    move |new_value: bool| {
                                        ...
                                        spawn(async move {
                                            match mechvibes_core::utils::auto_startup::set_auto_startup(new_value) {
                                                Ok(_) => {
                                        ...
                                        move |new_value: bool| {
                                            ...
                                            spawn(async move {
                                                if AppConfig::get().auto_start {
                                                    match mechvibes_core::utils::auto_startup::set_auto_startup(true) {
                                                        Ok(_) => {
