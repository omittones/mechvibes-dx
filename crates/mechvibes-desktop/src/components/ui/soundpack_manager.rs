...
use mechvibes_core::{AudioContext, audio::soundpack_loader::load_soundpack_from_config};
use dioxus::prelude::*;
    ...
    rsx! {
        div { class: "space-y-4",
                ...
                div { class: "flex items-center gap-4",
                    ...
                    if last_scan > 0 {
                        div { class: "text-xs text-base-content/60",
                            "Last scan: {mechvibes_core::utils::time::format_relative_time(last_scan)}"
                        }
                    }
                }
            }
            ...
            div { class: "space-y-2",
                ...
                div { class: "flex gap-2",
                    button {
                        class: "btn btn-soft btn-sm",
                        onclick: move |_| {
                            let builtin_keyboard_dir = mechvibes_core::state::paths::soundpacks::get_builtin_soundpacks_dir()
                                .join("keyboard");
                            let _ = mechvibes_core::utils::path::open_path(&builtin_keyboard_dir.to_string_lossy());
                        },
                    ...
                    button {
                        class: "btn btn-soft btn-sm",
                        onclick: move |_| {
                            let builtin_mouse_dir = mechvibes_core::state::paths::soundpacks::get_builtin_soundpacks_dir()
                                .join("mouse");
                            let _ = mechvibes_core::utils::path::open_path(&builtin_mouse_dir.to_string_lossy());
                        },
                        FolderOpen { class: "w-4 h-4 mr-1" }
                        "Mouse"
                    }
            ...
            div { class: "space-y-2",
                ...
                div { class: "text-sm text-base-content/70", "Add your own custom sound packs here." }
                div { class: "flex gap-2",
                    button {
                        class: "btn btn-soft btn-sm",
                        onclick: move |_| {
                            let custom_keyboard_dir = mechvibes_core::state::paths::soundpacks::get_custom_soundpacks_dir()
                                // Create the directory if it doesn't exist
                                .join("keyboard");
                            let _ = std::fs::create_dir_all(&custom_keyboard_dir);
                            let _ = mechvibes_core::utils::path::open_path(&custom_keyboard_dir.to_string_lossy());
                        },
                        FolderOpen { class: "w-4 h-4 mr-1" }
                        "Keyboard"
                    }
                    button {
                        class: "btn btn-soft btn-sm",
                        onclick: move |_| {
                            let custom_mouse_dir = mechvibes_core::state::paths::soundpacks::get_custom_soundpacks_dir()
                                // Create the directory if it doesn't exist
                                .join("mouse");
                            let _ = std::fs::create_dir_all(&custom_mouse_dir);
                            let _ = mechvibes_core::utils::path::open_path(&custom_mouse_dir.to_string_lossy());
                        },
                        FolderOpen { class: "w-4 h-4 mr-1" }
                        "Mouse"
                    }
