use mechvibes_core::utils::constants::{APP_NAME, APP_PROTOCOL, APP_PROTOCOL_URL};
use std::env;
...
#[cfg(target_os = "linux")]
pub fn register_protocol() -> Result<(), Box<dyn std::error::Error>> {
    use mechvibes_core::utils::constants::APP_NAME_LOWERCASE;
    use std::fs;
...
fn import_theme_from_protocol(theme_data: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    use mechvibes_core::state::config::AppConfig;
    use mechvibes_core::state::themes::CustomThemeData;
