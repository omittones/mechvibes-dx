...
fn find_keyboard_devices() -> Result<Vec<evdev::Device>, ()> {
...
pub fn start_evdev_keyboard_listener(
    ...
    thread::spawn(move || {
        ...
        let mut load_keyboards = true;
        let mut ctrl_pressed = false;
        let mut alt_pressed = false;

        let mut keyboards: Vec<evdev::Device> = Vec::new();
        loop {
            // load keyboards if needed
            if load_keyboards {
                keyboards = match find_keyboard_devices() {
            ...
            for device in &mut keyboards {
                ...
                match device.fetch_events() {
                    ...
                    Err(e) => {
                        log::error!("❌ Error fetching events: {:?}", e.kind());
                        // refresh keyboards if error
                        load_keyboards = true;
                        break;
                    }
                }
